...
class TrafficSignal:
    ...
    def _diff_CO2_reward(self):
        ts_C02 = sum(self.get_accumulated_CO2_per_lane()) / 1000.0
        reward = self.last_C02 - ts_C02
        self.last_C02 = ts_C02
        return reward
    ...
    def get_accumulated_C02_per_lane(self) -> List[float]:
        """Returns the accumulated waiting time per lane.
        ...
        for lane in self.lanes:
            ...
            for veh in veh_list:
                veh_lane = self.sumo.vehicle.getLaneID(veh)
                acc_C02 = self.sumo.vehicle.getCO2Emission(veh)
                if veh not in self.env.vehicles:
                    self.env.vehicles[veh] = {veh_lane: acc_C02}
                else:
                    self.env.vehicles[veh][veh_lane] = acc_C02 - sum(
                        [self.env.vehicles[veh][lane] for lane in self.env.vehicles[veh].keys() if lane != veh_lane]
                    )
                C02_measure += self.env.vehicles[veh][veh_lane]
            C02_per_lane.append(C02_measure)
        return C02_per_lane

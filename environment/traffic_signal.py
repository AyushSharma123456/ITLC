...
class TrafficSignal:
    ...
    def _fuel_reward(self):
        return -self.get_average_fuel()

    def _CO2_emissions_stopped(self):
        return -self.get_CO2_emissions_slow_vehicles()
    ...
    def _diff_CO2_reward(self):
        ts_CO2 = self.get_CO2_emissions()
        reward = self.last_CO2 - ts_CO2
        self.last_CO2 = ts_CO2
        return reward

    def _diff_CO2_slow_vehicles_reward(self):
        ts_slow_CO2 = self.get_CO2_emissions_slow_vehicles()
        reward = self.last_slow_CO2 - ts_slow_CO2
        self.last_slow_CO2 = ts_slow_CO2
        return reward
    ...
    def get_accumulated_waiting_time_per_lane(self) -> List[float]:
        ...
        """print("Waiting times for Stopped Vehicles:", wait_time_per_lane)"""
    ...
    def get_average_fuel(self) -> float:
        """Returns the average speed normalized by the maximum allowed speed of the vehicles in the intersection.
        ...
        if len(vehs) == 0:
            return 1.0
        for v in vehs:
            avg_fuel += self.sumo.vehicle.getFuelConsumption(v)
        return avg_fuel / len(vehs)
    ...
    def get_CO2_emissions_slow_vehicles(self):
        
        avg_emissions = 0.0
        slow_vehicles = [vehicle for vehicle in self._get_veh_list() if self.sumo.vehicle.getSpeed(vehicle) < 0.1]

        if len(slow_vehicles) == 0:
            return 0.0  # Return 0.0 for no emissions from slow vehicles

        for v in slow_vehicles:
            avg_emissions += self.sumo.vehicle.getCO2Emission(v)

        return avg_emissions / len(slow_vehicles)

    def get_tail_prob(self):
        max_penalty = 0  # Initialize the maximum penalty
        current_time = self.sumo.simulation.getTime()  # Get the current simulation time

        for vehicle_id in self._get_veh_list():
            current_speed = self.sumo.vehicle.getSpeed(vehicle_id)
            if current_speed < 0.1:  # If vehicle speed is less than 0.1 m/s
                departure_time = self.sumo.vehicle.getDeparture(vehicle_id)  # Get the departure time of the vehicle
                time_difference = current_time - departure_time
                max_penalty = max(max_penalty, time_difference)
    ...
    def get_lanes_density(self) -> List[float]:
        ...
        """print("Density per lane:", lanes_density)"""
        return [min(1, density) for density in lanes_density]
    ...
    def get_lanes_emissions(self) -> List[float]:
        """Returns the queue [0,1] of the vehicles in the incoming lanes of the intersection.
        ...
        """print("Emissions for Vehicles:", lanes_traffic)"""

        scaled_emissions = self.scale_values_to_range(lanes_traffic)
        scaled_emissions = [round(number, 1) for number in scaled_emissions]
        return scaled_emissions

    def get_total_stopped_emissions_per_lane(self) -> List[float]:
        """Calculates total emissions per lane for stopped vehicles."""
        total_emissions_per_lane = []

        for lane in self.lanes:
            vehicles_in_lane = self.sumo.lane.getLastStepVehicleIDs(lane)
        
            # Initialize total emissions for this lane
            total_emissions_in_lane = 0.0
        
            for vehicle_id in vehicles_in_lane:
                vehicle_speed = self.sumo.vehicle.getSpeed(vehicle_id)
            
                # Check if the vehicle is stopped (speed less than a threshold)
                if vehicle_speed < 0.1:  # Adjust the threshold as needed
                    # Retrieve and add the emissions of this stopped vehicle
                    vehicle_emissions = self.sumo.vehicle.getCO2Emission(vehicle_id)
                    total_emissions_in_lane += vehicle_emissions
        
            # Append the total emissions for this lane to the result list
            total_emissions_per_lane.append(total_emissions_in_lane)

        """print("Emissions for Stopped Vehicles:", total_emissions_per_lane)"""

        scaled_stopped_emissions = self.scale_values_to_range(total_emissions_per_lane)
        scaled_stopped_emissions = [round(number, 1) for number in scaled_stopped_emissions]
        """print("Emissions for Stopped Vehicles:", scaled_stopped_emissions)"""
        return scaled_stopped_emissions

    def scale_values_to_range(self, values: List[float]) -> List[float]:
        """Scales a list of values to the range [0, 1]."""
        min_value = min(values)
        max_value = max(values)
        
        scaled_values = [
            (value - min_value) / max((max_value - min_value),1)
            for value in values
        ]
    ...
    reward_fns = {
        ...
        "diff-emissions": _diff_CO2_reward,
        "diff-stopped-emissions": _diff_CO2_slow_vehicles_reward,
        "fuel-consumption": _fuel_reward,
        "stopped-emissions": _CO2_emissions_stopped,
        "tailed-probabilities":_tail_prob,
    }

...
class SumoEnvironment(gym.Env):
    ...
    def __init__(
        ...
        self.vehicle_info_df = pd.DataFrame(columns=["Step", "Added Vehicles", "Finished Vehicles"])
    ...
    def reset(self, seed: Optional[int] = None, **kwargs):
        ...
        # Initialize added and finished vehicles counters
        self.added_vehicles_count = 0
        self.finished_vehicles_count = 0

        if self.episode != 0:
            self.close()
            self.save_csv(self.out_csv_name, self.episode)
            # Export the vehicle information DataFrame to an Excel file
            excel_filename = f"vehicle_info_ep{self.episode}.xlsx"
            self.vehicle_info_df.to_excel(excel_filename, index=False)
            print(f"Vehicle information exported to {excel_filename}")
        self.episode += 1
        self.metrics = []
    ...
    def _run_steps(self):
        ...
        while not time_to_act:
            self._sumo_step()
            # Logic to update the vehicle information DataFrame
            self.added_vehicles_count += self.sumo.simulation.getDepartedNumber()
            self.finished_vehicles_count += self.sumo.simulation.getArrivedNumber()

            # Append information to the DataFrame
            step_info = {
                "Step": int(self.sim_step),
                "Added Vehicles": self.added_vehicles_count,
                "Finished Vehicles": self.finished_vehicles_count,
            }
            self.vehicle_info_df = self.vehicle_info_df._append(step_info, ignore_index=True)
            for ts in self.ts_ids:
    ...
    def _compute_info(self):
        ...
        info["added_vehicles_count"] = self.added_vehicles_count
        info["finished_vehicles_count"] = self.finished_vehicles_count
    ...
    def _get_system_info(self):
        ...
        teleported_vehicles = self.sumo.vehicle.getTeleportingIDList()
        ...
        emissions_stopped = [self.sumo.vehicle.getCO2Emission(vehicle) for vehicle in stopped_vehicles]
        vehicles_with_delay_more_than_2_minutes = 0
        max_waiting_time = max(waiting_times) if waiting_times else 0
        
        for vehicle in vehicles:
            # Check if the vehicle is known to SUMO
            departure_delay = self.sumo.vehicle.getDepartDelay(vehicle)
        ...
        return {
            ...
            "system_max_waiting_time": max_waiting_time,
            ...
            "vehicles_with_delay_more_than_2_minutes": vehicles_with_delay_more_than_2_minutes,
